...
type RestOf<T extends IClient | NewClient> = Omit<T, 'datedebut' | 'datefin' | 'datecreation' | 'datevalidation'> & {
...
export class ClientService {
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/clients', 'erpltedb');
  ...
  protected convertDateFromClient<T extends IClient | NewClient | PartialUpdateClient>(client: T): RestOf<T> {
    return {
      ...client,
      datedebut: client.datedebut?.toJSON() ?? null,
      datefin: client.datefin?.toJSON() ?? null,
      datecreation: client.datecreation?.toJSON() ?? null,
      datevalidation: client.datevalidation?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restClient: RestClient): IClient {
    return {
      ...restClient,
      datedebut: restClient.datedebut ? dayjs(restClient.datedebut) : undefined,
      datefin: restClient.datefin ? dayjs(restClient.datefin) : undefined,
      datecreation: restClient.datecreation ? dayjs(restClient.datecreation) : undefined,
      datevalidation: restClient.datevalidation ? dayjs(restClient.datevalidation) : undefined,
    };
  }
